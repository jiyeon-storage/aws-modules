...
resource "aws_lb_target_group_attachment" "this" {
  for_each = { for k, v in var.target_groups : k => v if local.create && lookup(v, "create_attachment", true) && try(v.target_id, "") != "" }
...
resource "aws_lb_target_group_attachment" "additional" {
  for_each = { for k, v in var.additional_target_group_attachments : k => v if local.create && try(v.target_id, null) != null }
...
#   security_group_name = format("%s-sg-%s-kr-%s-internal", var.prefix, var.env, var.purpose)
